...
#define FREG   4 // there are 4 GPRs (R0-R3)
...
typedef struct {
    union {
        ...
        struct {
            uint8_t ex         : 4;
            uint8_t fr2        : 2; //   10 bits used
            uint8_t fr1        : 2;
            uint8_t dr         : 2;
            uint8_t flag       : 2; // flag for 0,1,2,3. 0 is ld, 1 is st, 2, is add, 3 is display.
            uint8_t opcode     : 4; // will be opcode 13
        } __attribute__((packed));
    } __attribute__((packed));
} __attribute__((packed)) flt_case_t;
...
typedef struct {
    ...
    address_t freg[FREG];   // this is the floating point register
